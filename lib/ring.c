...
void ring_stat(struct chring *r, int nr)
{
    ...
    for (i = 0; i < r->used; i++) {
        if (i == 0) {
            rglen[r->array[i].site_id] += r->array[i].point;
            rglen[r->array[i].site_id] += (
                0xffffffffffffffff - r->array[r->used].point);
        } else {
            rglen[r->array[i].site_id] += r->array[i].point - last_point;
        }
...
int ring_add(struct chring *r, char *node, int port, int id, int vid_max)
{
    ...
    for (i = 0; i < vid_max; i++) {
        snprintf(buf, 256, "%s.%d.%d.%d", node, port, id, i);
        (p + i)->point = hvfs_hash(buf, strlen(buf));
        (p + i)->vid = i;
        (p + i)->type = CHP_AUTO;
        (p + i)->site_id = id;
        (p + i)->node = strdup(node);
        (p + i)->port = port;
