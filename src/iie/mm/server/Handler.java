            ...
            while (true) {
                ...
                switch (header[0]) {
                    case ActionType.SYNCSTORE: {
                        ...
                        String result = null;
                        try {
                            result = sp.storePhoto(set, md5, bytes, clen, fnLen + setlen + md5len, fLen, fn);
                    ...
                    case ActionType.BSEARCH: {
                        // 这样能把byte当成无符号的用，拼接的元信息长度最大可以255
                        int infolen = header[1] & 0xff;
                        if (infolen > 0) {
                            String infos = new String(readBytes(infolen, dis), 0,
                                    infolen);
                            long setoff = dis.readLong();
                            int length = dis.readInt();

                            ObjectContent oc = null;
                            try {
                                oc = sp.searchPhoto(infos, null,
                                        threadLocalSendBuffer.get(), setoff, length);
                            } catch (RedirectException e) {
                            }
                            // FIXME: ??zhaoyang 有可能刚刚写进redis的时候，还无法马上读出来,
                            // 这时候会无法找到图片,返回null
                    ...
                    case ActionType.XSEARCH: {
                        ...
                        if (setlen > 0 && md5len > 0) {
                            ...
                            ObjectContent oc = null;
                            try {
                                oc = sp.getPhoto(set, md5, offset, length);
                            } catch (RedirectException e) {
