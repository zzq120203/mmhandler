...
public class HTTPHandler extends AbstractHandler {
    ...
    private void okResponse(Request baseRequest, HttpServletResponse response, byte[] content, boolean sc_ok, int off, long flen, boolean range) throws IOException {
	...
	private void redirectToSz(Request baseRequest, HttpServletResponse response, String key) throws IOException {
    		String url = "10.144.16.56:20099";
       		System.out.println("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz");
		response.setContentType("text/plain;charset=utf-8");
		response.setHeader("Access-Control-Allow-Origin", "*");
        	baseRequest.setHandled(true);        
        	response.setStatus(HttpServletResponse.SC_MOVED_TEMPORARILY);
        	response.sendRedirect("http://" + url + "/get?key=" + key);
        	response.getWriter().flush();
	}
	...
	if (range != null)
		...
		if (infos.length == 2) {
			...
			byte[] infobyte = null;
                        try {
                        	infobyte = sp.getFileInfo(infos[0], infos[1]);
                        } catch (NullPointerException e) {
				System.out.println("NullPointerException "+ key);
                                redirectToSz(baseRequest, response, key);
                        }
                    	if (infobyte == null)
                    		redirectToSz(baseRequest, response, key);
			else {
			...
			if ('#'==info.charAt(1))
                        ...
                        long flen = Long.parseLong(info.split("@|#")[5]) - offset;
                        int elen = conf.getElen();
			int s = (int) Math.floor(flen / elen);
                        for(int i = 0; i <= s; i ++ ){
                            if (i != s) {
                                oc = sp.getPhoto(infos[0], infos[1], i * elen + offset, elen);
                            } else {
                                oc = sp.getPhoto(infos[0], infos[1], i * elen + offset, (int) (flen % elen));
                            }
					...
					//response.setHeader("Content-Disposition", "attachment;filename="+key+".jpg");
                                        break;
                                    case 't':
                                        response.setContentType("text/plain;charset=utf-8");
					...
					response.setHeader("Content-Disposition", "attachment;filename="+key+".wav");
                                        break;
                                    case 'v':
                                        response.setContentType("video/mp4");
					response.setHeader("Content-Disposition", "attachment;filename="+key+".mp4");
                                        break;
                                    default:
                                        response.setContentType("text/plain;charset=utf-8");
                                }
                                okResponse(baseRequest, response, Arrays.copyOfRange(oc.content, 0, oc.length), (i == s), offset, flen + offset, (range != null));
                            }
                        }}
                    } catch (RedirectException e) {
                        redirectResponse(baseRequest, response, e);
                    }
                } else if (infos.length == conf.getVlen()) {
		     ObjectContent oc = null;
                    try {
                        long flen = Long.parseLong(infos[5]) - offset;
                        int elen = conf.getElen();
			int s = (int) Math.floor(flen / elen);
                        for(int i = 0; i <= s; i ++ ){
                            if (i != s) {
                                oc = sp.searchPhoto(key, null, null, i * elen + offset, elen);
                            } else {
                                oc = sp.searchPhoto(key, null, null, i* elen + offset, (int) (flen % elen));
                            }
                            if (oc == null || oc.content == null || oc.length == 0) {
                                notFoundResponse(baseRequest, response, "#FAIL:can not find any MM object by key=" + key);
                               	//System.out.println("重定向到SZ!");
				 //redirectToSz(baseRequest, response, infos[0] + "@" + infos[1]);
                            } else {
    ...
    private String convertToDate(long ts) {
        ...
        try {
            ...
            String page = "<HTML> " +
                    ...
                    "</HEAD>" +
                    "<BODY>" +
		    "<H2><a href=\"http://10.136.140.146:20099/m/play.html\"> MMS Play</a></H2>" +
    ...
    private String nameBaseToString(Map<String, String> namebase) {
        ...
        for (Map.Entry<String, String> entry : namebase.entrySet()) {
...
//				"</div></div>" +
				...
				"<button class=\"btn btn-info\" type=\"button\" onClick=\"loadXMLDoc(xmldoc.value)\"><i class=\"fa fa-search\"></i></button>" +
    ...
    private void doBBrowse(String target, Request baseRequest, HttpServletRequest request,
		...
		if (set == null) {
			...
			if (set.contains("@")) {
				...
				elements = sp.getSetElements(set);
			if (elements == null) {
                        	response.setContentType("text/html;charset=utf-8");
                        	response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
                       		baseRequest.setHandled(true);
